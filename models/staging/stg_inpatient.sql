...
with base as (
    select
        ...
        round(try_to_number(nullif(trim(INSCCLAIMAMTREIMBURSED), '')) / 88) * 10 as insclaimamtreimbursed,
        round(try_to_number(nullif(trim(DeductibleAmtPaid),       '')) / 88) * 10 as deductibleamtpaid,
