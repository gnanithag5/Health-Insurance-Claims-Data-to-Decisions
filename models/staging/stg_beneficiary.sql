...
with base as (
    select
        ...
        iff(
          regexp_like(RenalDiseaseIndicator, '^\s*y\s*$', 'i'),
          1, 
          0
        ) as renal_disease_indicator,
        ...
        -- INR → USD (/88, rounded)
        round(try_to_number(nullif(trim(IPAnnualReimbursementAmt), '')) / 88) * 10  as IPAnnualReimbursementAmt,
        round(try_to_number(nullif(trim(IPAnnualDeductibleAmt),   '')) / 88) * 10  as IPAnnualDeductibleAmt,
        round(try_to_number(nullif(trim(OPAnnualReimbursementAmt), '')) / 88) * 10 as OPAnnualReimbursementAmt,
        round(try_to_number(nullif(trim(OPAnnualDeductibleAmt),   '')) / 88) * 10  as OPAnnualDeductibleAmt
